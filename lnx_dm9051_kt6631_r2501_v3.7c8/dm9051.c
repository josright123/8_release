...
struct driver_config {
    ...
    struct {
		int ndelayF;
		int nTargetMaxNum;
		unsigned long delayF[SCHED_MAXNUM];
	} sched;
} confdata = {
...
static unsigned int get_tx_free(struct board_info *db) {
    ...
    //dm9051_get_reg(db, DM9051_TXFSSR, &rb);
	ret = regmap_read(db->regmap_dm, DM9051_TXFSSR, &rb);
	if (ret < 0) {
...
//static dm9051_tx_conti_poll(struct board_info *db)
//{
//	int ret;
//	return ret;
//}
...
static void dm9051_write_crypt_word(struct board_info *db, u8 crypt_word) {
	...
	if (ret)
		return;
	//crypt_1 = std_spi_read_reg(db, DM9051_PIDL);
	//crypt_2 = std_spi_read_reg(db, DM9051_PIDH);
	...
	if (ret)
		return;
	//std_spi_write_reg(db, 0x49, crypt_1);
	//std_spi_write_reg(db, 0x49, crypt_2);
	ret = dm9051_set_reg(db, 0x49, crypt_word);
	if (ret)
		return;
	//std_spi_write_reg(db, 0x49, crypt_word);
