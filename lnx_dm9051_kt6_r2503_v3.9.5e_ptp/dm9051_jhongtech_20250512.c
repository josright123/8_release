...
static int dm9051_core_reset(struct board_info *db)
{
	...
	if (ret)
		return ret;
	ret = regmap_write(db->regmap_dm, DM9051_PPCR, 0xF); /* Pause Count, PPCR_PAUSE_COUNT */
	if (ret)
		return ret;
	...
	return dm9051_set_reg(db, DM9051_INTCR, dm9051_init_intcr_value(db));
}
...
static int dm9051_loop_rx(struct board_info *db)
{
	...
	do {
		...
		if (SCAN_BH(rxbyte) != DM9051_PKT_RDY)
			break; /* exhaust-empty */
...
static irqreturn_t dm9051_rx_threaded_plat(int irq, void *pw) //dm9051_rx_threaded_irq
{
...
static int dm9051_open(struct net_device *ndev)
{
	...
	ret = request_threaded_irq(spi->irq, NULL, /*dm9051_rx_threaded_irq*/ dm9051_rx_threaded_plat,
