...
const struct eng_config *dm9051_defaultdata = &driver_default_mode; //econf
...
static void SHOW_OPTION_MODE(struct spi_device *spi)
{
	...
	dev_info(dev, "Check TX End: %llu, TX mode= %s mode, DRVR= %s, %s\n", dm9051_defaultdata->tx_timeout_us, dmplug_tx,
			dm9051_defaultdata->force_monitor_rxb ? "monitor rxb" : "silence rxb",
			dm9051_defaultdata->force_monitor_tx_timeout ? "monitor tx_timeout" : "silence tx_ec");
}
...
int dm9051_nsr_poll(struct board_info *db)
{
	...
	ret = regmap_read_poll_timeout(db->regmap_dm, DM9051_NSR, mval,
								   ...
								   1, dm9051_defaultdata->tx_timeout_us); // 2100 <- 20
	if (dm9051_defaultdata->force_monitor_tx_timeout && ret == -ETIMEDOUT)
...
{
	if (dm9051_defaultdata->force_monitor_rxb)
	{
...
{
	if (dm9051_defaultdata->force_monitor_rxc && scanrr && db->bc.nRxcF < 25)
	{
		db->bc.nRxcF += scanrr;
		SHOW_MONITOR_RXC(db);
	}
...
void dm9051_poll_servicep(struct work_struct *work) //.dm9051_poll_delay_plat()
{
	...
	if (db->bc.ndelayF >= dm9051_defaultdata->sched.nTargetMaxNum)
		db->bc.ndelayF = POLL_OPERATE_INIT;

	schedule_delayed_work(&db->irq_workp, dm9051_defaultdata->sched.delayF[db->bc.ndelayF++]);
}
#endif
