...
{
    return (get_dts_irqf(db) == IRQF_TRIGGER_LOW || get_dts_irqf(db) == IRQF_TRIGGER_FALLING) ? INTCR_POL_LOW
                                                                                              : INTCR_POL_HIGH;
}
...
static int dm9051_update_bits(struct board_info *db, unsigned int reg, unsigned int mask, unsigned int val)
{
    int ret;
...
static int dm9051_set_regs(struct board_info *db, unsigned int reg, const void *val, size_t val_count)
{
    int ret;
...
static int dm9051_get_regs(struct board_info *db, unsigned int reg, void *val, size_t val_count)
{
    int ret;
...
int dm9051_ncr_poll(struct board_info *db)
{
    ...
    ret = regmap_read_poll_timeout(db->regmap_dm, DM9051_NSR, mval, !(mval & NCR_RST), 10, 100);
    if (ret == -ETIMEDOUT)
...
int dm9051_nsr_poll(struct board_info *db)
{
    ...
    ret = regmap_read_poll_timeout(db->regmap_dm, DM9051_NSR, mval, mval & (NSR_TX2END | NSR_TX1END), 1,
                                   param->tx_timeout_us); // 2100 <- 20
...
static int dm9051_epcr_poll(struct board_info *db)
{
    ...
    ret = regmap_read_poll_timeout(db->regmap_dm, DM9051_EPCR, mval, !(mval & EPCR_ERRE), 100, 10000);
    if (ret == -ETIMEDOUT)
...
static int dm9051_set_rcr(struct board_info *db) { return dm9051_set_reg(db, DM9051_RCR, db->rctl.rcr_all); }
...
static int regmap_spi_async_write(void       *context, //.V510_COMPLEX
                                  const void *reg, size_t reg_len, const void *val, size_t val_len,
...
static int regmap_spi_read(void *context, const void *reg, size_t reg_size, void *val, size_t val_size)
{
    struct device *dev = context;
    struct spi_device *spi = to_spi_device(dev);
...
static const struct regmap_bus *regmap_get_spi_bus(struct spi_device *spi, const struct regmap_config *config)
{
    return &regmap_bus_dm;
}

struct regmap *__devm_regmap_init_spi_dm(struct spi_device *spi, const struct regmap_config *config,
                                         struct lock_class_key *lock_key, const char *lock_name)
{
    ...
    #define devm_regmap_init_spi(dev, config) __regmap_lockdep_wrapper(__devm_regmap_init_spi_dm, #config, dev, config)
#endif
...
static int dm9051_get_eeprom_len(struct net_device *dev) { return 128; }

static int dm9051_get_eeprom(struct net_device *ndev, struct ethtool_eeprom *ee, u8 *data)
{
    struct board_info *db     = to_dm9051_board(ndev);
...
static int dm9051_set_eeprom(struct net_device *ndev, struct ethtool_eeprom *ee, u8 *data)
{
    struct board_info *db     = to_dm9051_board(ndev);
    int                offset = ee->offset;
    int                len    = ee->len;
...
static void dm9051_get_pauseparam(struct net_device *ndev, struct ethtool_pauseparam *pause)
{
    struct board_info *db = to_dm9051_board(ndev);
...
static int dm9051_set_pauseparam(struct net_device *ndev, struct ethtool_pauseparam *pause)
{
    struct board_info *db = to_dm9051_board(ndev);
...
static void dm9051_get_ethtool_stats(struct net_device *ndev, struct ethtool_stats *stats, u64 *data)
{
    struct board_info *db = to_dm9051_board(ndev);
    int i;
...
int rx_head_break(struct board_info *db)
{
    ...
    u8 err_bits = PTP_STATUS_BITS(db); /* 7 rxhead ptpc, when REG60H.D[0]=0, PTP Function enable, or REG61H.D[0]=1, and
                                          D[1]=0, then re-defined RSR */
    ...
    if (db->rxhdr.status & err_bits || rxlen > DM9051_PKT_MAX)
    {
        netif_warn(db, rx_status, db->ndev, "Err: [dm9.Monitor headbyte/status/rxlen %2x %2x %04x]\n",
                   db->rxhdr.headbyte, db->rxhdr.status, db->rxhdr.rxlen);
    ...
    while (!skb_queue_empty(&db->txq))
    {
        ...
        if (skb)
        {
            ...
            if (ret)
            {
                ...
                if (netif_queue_stopped(ndev) && (skb_queue_len(&db->txq) < DM9051_TX_QUE_LO_WATER))
...
static int dm9051_req_irq(struct board_info *db, irq_handler_t handler)
{
    ...
    ret = request_threaded_irq(spi->irq, NULL, handler, //'dm9051_rx_threaded_plat'
                               get_dts_irqf(db) | IRQF_ONESHOT, db->ndev->name, db);
    if (ret < 0)
...
static int dm9051_set_features(struct net_device *ndev, netdev_features_t features)
{
    struct board_info *db = to_dm9051_board(ndev);
...
static int dm9051_mdio_register(struct board_info *db)
{
    ...
    snprintf(db->mdiobus->id, MII_BUS_ID_SIZE, "dm9051-%s.%u", dev_name(&spi->dev), spi->chip_select);
...
static void dm9051_handle_link_change(struct net_device *ndev)
{
    ...
    netif_warn(db, link, db->ndev, "%s",
               db->phydev->link ? "Link is Up" : "Link is Down"); // phy_print_status(db->phydev);
...
static int dm9051_phy_connect(struct board_info *db)
{
    ...
    snprintf(phy_id, sizeof(phy_id), PHY_ID_FMT, db->mdiobus->id, DM9051_PHY_ADDR);
...
#if LINUX_VERSION_CODE <= KERNEL_VERSION(5, 10, 0) || LINUX_VERSION_CODE <= KERNEL_VERSION(5, 10, 11)
