...
根據不同平台或功能需求，通過巨集（宏）定義來啟用或禁用特定的驅動功能。其主要內容如下：

- 透過 `#define` 和 `#ifdef` 控制條件編譯，根據是否定義某些巨集來啟用對應的功能。
- `PLUG_ENABLE_INT` 相關區塊：啟用中斷（INT）相關功能，並可進一步細分為 CLKOUT、TWO_STEP 等子功能（但預設註解掉）。
