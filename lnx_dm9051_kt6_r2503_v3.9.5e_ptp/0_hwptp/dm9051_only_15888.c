...
static const struct ethtool_ops dm9051_ethtool_ops = {
	...
	//15888, .get_ts_info = dm9051_ts_info,
};
...
#endif			
			db->ptp_mode = 0; //15888, dm9051_ptp_one_step(skb);
...
//15888, one-step can, two-step can't
		result_tx = dm9051_loop_tx(db); /* more tx better performance */
		if (result_tx < 0)
			goto out_unlock;
		// Spenser
...
static int dm9051_netdev_ioctl(struct net_device *ndev, struct ifreq *rq, int cmd)
{
    ...
    //struct hwtstamp_config config; //15888, what related some where !?
...
static const struct net_device_ops dm9051_netdev_ops = {
	...
	//15888, .ndo_eth_ioctl = dm9051_netdev_ioctl,
};
