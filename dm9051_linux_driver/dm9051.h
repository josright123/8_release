...
#include <linux/kernel.h> // 包含 printk() 的定義
...
// 定義我們的level
#define DM_DEBUG_LEVEL KERN_INFO
// 定義 dm_printk
#define dm_printk(fmt, ...) printk(DM_DEBUG_LEVEL fmt "\n", ##__VA_ARGS__)
// 定義 bd->dbg_flags
#define DP_REG_WRITE BIT(0)

#define DM9051_ID		0x9051
#define DM9058_ID		0x9058
...
#define TCR_TS_EN               BIT(7)
#define TCR_DIS_JABBER_TIMER    BIT(6) // for Jabber Packet support
#define TCR_EXCECM              BIT(5)
#define TCR_PAD_DIS2            BIT(4)
#define TCR_CRC_DIS2            BIT(3)
#define TCR_PAD_DIS1            BIT(2)
#define TCR_CRC_DIS1            BIT(1)
...
// PTP use the same bit, physical layer error
#define RSR_PARITY              BIT(3)
#define RSR_AE			BIT(2)
...
#define RSR_ERR_BITS_PTP			\
	(RSR_RF | RSR_RWTO | RSR_CE | RSR_FOE)
//0x0A
...
struct board_info {
	...
	// switch to enable PTP functions
	int                             ptp_on;
	struct ptp_clock                *ptp_clock;
	struct ptp_clock_info           ptp_caps;
	struct ptp_pin_desc             pin_desc[N_PINS];
	struct work_struct              ptp_extts_work;
	struct sk_buff                  *ptp_tx_skb;
	struct hwtstamp_config          tstamp_config;
	// config result for set timestamp mode
	u32                             rx_ctl;
	// save ioctl SIOCSHWTSTAMP tx timestamp mode,
        // bit shifting of enum hwtstamp_config.flags
	u32                             tx_ctl;
	u32                             rx_cfg;
	bool                            is_l4;
	bool                            is_l2;
	...
	unsigned int                    extts_flags[2]; // index 0: GP1, index 1: GP2
        // GPIO1, GPIO2 polarity, 0: low active, 1: high active
        int                             gpio_polarity[2];
	// copy from skb::tx_flags, bits defined in skbuff.h
	unsigned int                    ptp_tx_flags;
	unsigned int                    ptp_class;
	struct ptp_header*              ptp_hdr;
	u8                              ptp_msgtype;
	struct system_time_snapshot     snapshot;
	s64                             last_rate;  //last rate used in adjfine()
...
int dm9051_get_reg(struct board_info *db, unsigned int reg, unsigned int* val);
