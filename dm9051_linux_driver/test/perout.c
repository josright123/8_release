...
int main() {
    ...
    // 設定從現在起 2 秒後開始，並每 1 秒觸發一次
    time_t now = time(NULL);
    perout.index = 1;
    perout.start.sec = now + 2;
    perout.start.nsec = 0;
    perout.period.sec = 1;
    perout.period.nsec = 0; // 10^7 ns = 10 ms
    perout.flags = PTP_PEROUT_DUTY_CYCLE; // 使用 duty cycle 模式
    perout.on.sec = 0;
    perout.on.nsec = 800000000; // 800 ms duty cycle
