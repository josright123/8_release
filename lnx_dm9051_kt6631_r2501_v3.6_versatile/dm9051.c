...
#define LNX_DM9051_RELEASE_VERSION		"lnx_dm9051_kt6631_r2501_v3.x" //note only
...
typedef struct {
	...
	int build_conf;	//build, 'software_build_kernel_conf'
	int os64; //aarch, [structure: for os data bus bits]
    struct {
		char *kernel_ver_brief;  //note, 'software_build_kernel_conf'
		char *verify_info;
    } dm9051note;
    struct {
    ...
    struct {
		int LXR_select; //lxr_info, 'morden_lxr_kernel_conf'
    } softopt;
} driver_cfg_t;

static const driver_cfg_t kernel_conf_5 = {
    "lnx_dm9051 kt6631_driver",
    "lnx_dm9051_kt6631_r2501_v3.5f", //previous was "r2501.v3.5e",
    DM9051_KERNEL_5_10,
    AARCH_OS_32,
    { "KT5.10", "test in rpi4 bcm2711"},
...
static const driver_cfg_t kernel_conf_5_15 = {
    "lnx_dm9051 kt6631_driver",
    "lnx_dm9051_kt6631_r2501_v3.6c",
    DM9051_KERNEL_5_15,
    AARCH_OS_64,
    { "KT5.15", "test in rpi4 bcm2711"},
    { FORCE_MONITOR_RXB, FORCE_MONITOR_TXEC},
    { BURST_FULL_CONF, 0, 0},
    { 2100},
    { MORDEN_LXR_CONF},
...
static const driver_cfg_t kernel_conf_6_1 = {
    "lnx_dm9051 kt6631_driver",
    "lnx_dm9051_kt6631_r2501_v3.6c", //previous was "r2501.v3.5e",
    DM9051_KERNEL_6_1,
    AARCH_OS_64,
...
static const driver_cfg_t kernel_conf_6_6_31 = {
    ...
    { FORCE_MONITOR_RXB, FORCE_MONITOR_TXEC},
    { BURST_ALIGNMENT_CONF, 32, 32}, //[32, 64].ok //64, 64
    { 2100},
    { MORDEN_LXR_CONF},
};

/*static const driver_cfg_t kernel_conf_6 = {
    "lnx_dm9051 kt6631_driver",
    "lnx_dm9051_kt6631_r2501_v3.5f", //previous was "r2501.v3.5e",
    DM9051_KERNEL_6_6,
    AARCH_OS_64,
...
static const driver_cfg_t *kernel_conf = &kernel_conf_6_6_31; //&kernel_conf_5_15
...
/* KERNEL_BUILD_CONF.DM9051_KERNEL_6_1 */
			    ...
			    size_t len)
{
	...
	if (kernel_conf->align.burst_mode) { //tx
		...
		if (kernel_conf->softopt.LXR_select >= DM9051_KERNEL_6_6) { //#if BOARD_CONF 
			//[dbg] bilk.wr
			const u8 *p = (const u8 *) buff;
			//[dbg] bilk.wr
			size_t BLKTX = kernel_conf->align.tx_blk; //SizeAlignmentTX	32 //256.NG //32 //64 //1024 //32(OK)
			while (len >= BLKTX) {
			   ...
			   size_t len)
{
	...
	if (kernel_conf->align.burst_mode) { //rx
		...
		if (kernel_conf->softopt.LXR_select >= DM9051_KERNEL_6_6) { //#if BOARD_CONF 
			...
			//[dbg] bulk.read
			//
			size_t BLKRX = kernel_conf->align.rx_blk; //SizeAlignmentRX; //64 //32 //256.NG //32 //1 //32(OK)
			while (len >= BLKRX) {
				ret = regmap_noinc_read(db->regmap_dm, reg, p, BLKRX);
				if (ret < 0) {			
					#if AARCH_OS_BITS //"_KERNEL_BUILD_CONF >= DM9051_KERNEL_5_15"
...
static int dm9051_map_chipid(struct board_info *db)
{
	...
	#if 1
	dev_info(dev, "LXR select: %s, KERNEL: %s\n",
		MORDEN_LXR_CONF == DM9051_KERNEL_5_10 ? "DM9051_KERNEL_5_10" :
		MORDEN_LXR_CONF == DM9051_KERNEL_5_15 ? "DM9051_KERNEL_5_15" :
		MORDEN_LXR_CONF == DM9051_KERNEL_6_1 ? "DM9051_KERNEL_6_1" :
		MORDEN_LXR_CONF == DM9051_KERNEL_6_6 ? "DM9051_KERNEL_6_6" :
		"UNKNOW",
		KERNEL_BUILD_CONF == DM9051_KERNEL_5_10 ? "DM9051_KERNEL_5_10" :
		KERNEL_BUILD_CONF == DM9051_KERNEL_5_15 ? "DM9051_KERNEL_5_15" :
		KERNEL_BUILD_CONF == DM9051_KERNEL_6_1 ? "DM9051_KERNEL_6_1" :
		KERNEL_BUILD_CONF == DM9051_KERNEL_6_6 ? "DM9051_KERNEL_6_6" :
		"UNKNOW");
	dev_info(dev, "NOTE: %s %s\n", kernel_conf->dm9051note.kernel_ver_brief, kernel_conf->dm9051note.verify_info);
...
{
	if (kernel_conf->dm9051mm.force_monitor_rxb) { //FORCE_MONITOR_RXB
...
static void dm9051_irq_delayp(struct work_struct *work)
{
	...
	schedule_delayed_work(&db->irq_workp, DM_TIMER_EXPIRE2); //DM_TIMER_EXPIRE1
}
...
static int dm9051_probe(struct spi_device *spi)
{
	...
	printk("\n");
	dev_info(dev, "Davicom: %s", kernel_conf->driver_release_version_candidate);
