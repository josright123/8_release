...
struct driver_config confdata = {
	"lnx_dm9051_kt6631_r2502_v3.7c10b",
	...
	FORCE_SILENCE_RXB,
	FORCE_MONITOR_RX_COUNT,
	FORCE_SILENCE_TX_TIMEOUT,
	0,
...
static int dm9051_nsr_poll(struct board_info *db)
{
	...
	if (kconf->force_monitor_tx_timeout && ret == -ETIMEDOUT)
...
//static int dm9051_phyread(void *context, unsigned int reg, unsigned int *val)
static int dm9051_phyread_log(struct board_info *db, char *head, int addr,
							  unsigned int reg, unsigned int *val)
{
	...
	do {
		...
		if (*val != bmsr) {
			printk("%s_read.ne, phyaddr %d [BMSR] %04x\n", head, addr, *val);
			bmsr = *val;
		} else {
			if (!(*val & BIT(6)))
				printk("%s_read.eq, phyaddr %d [BMSR] %04x\n", head, addr, *val);
			...
			if (*val == 0xffff) {
				...
				if (ret)
					return ret;
				printk("%s_read.ck[_check_], phyaddr %d phyreg(2) %04x\n", head, addr, vval);
			}
		}
	} while(0);
#endif
...
static int dm9051_mdio_read(struct mii_bus *bus, int addr, int regnum)
{
	...
	if (addr == DM9051_PHY_ADDR) {
		if (regnum == 1)
			ret = dm9051_phyread_log(db, "_mdio", addr, regnum, &val);
		else
...
static int dm9051_map_chipid(struct board_info *db)
{
	...
	dev_info(dev, "DRVR= %s, %s\n",
		...
		kconf->force_monitor_tx_timeout ? "monitor tx_timeout" : "silence tx_ec");
	return 0;
}
