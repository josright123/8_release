...
static int dm9051_core_reset0(struct board_info *db) //handle_link_change()'s, dm9051_open()'s instead. 
{
	int ret;
...
static int dm9051_core_reset_up(struct board_info *db) //dm9051_open()'s, handle_link_change()'s instead.
{
	int ret;

	printk("dm9051_core_reset_up\n");
...
//	phy_start(db->phydev);
	...
	ret = dm9051_set_interrupt_regs(db);
	if (ret)
		return ret;
